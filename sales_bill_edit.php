...
<form action="" method="post">
    ...
    <select name="bill" class="text-feild" style="width:205px;">
        ...
        <?php 
            $qurey="SELECT * FROM sales_bill;";
            $result=mysqli_query($conn,$qurey);
            while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
                ?>
                
                    <option value="<?php echo $row['bill_no']?>"> Bill NO :<?php echo $row['bill_no']?>-<?php echo $row['shop_name']; ?></option>
...
if(isset($_POST['submit'])){
    $bill=$_POST['bill'];
    $sql="SELECT * FROM sales_bill WHERE bill_no='$bill'";
    ...
    <form action="" method="POST" class="cus_update_form">
        <div class="cus_update">
           ...
           <button class="button1"> <a href="./delete_bill1.php?bill_no=<?php echo $bill; ?>">Delete Bill</a></button>
        </div>
    </form>
</div>
...
if(isset($_POST['date_submit'])){
    $date=$_POST['date'];
    $bill=$_POST['sname'];
    $sql2="UPDATE sales_bill SET bill_date='$date' WHERE bill_no='$bill'";
...
if(isset($_POST['amt_submit'])){
    ...
    $sql3="UPDATE sales_bill SET amt_withoutgst='$amt', net_amt='$net_amt' WHERE bill_no='$bill'";
